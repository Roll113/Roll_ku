...
# 常变量/同值化处理
def const_delete(df,col_list,threshold=None,target = None):
    """
    df:数据集
    col_list:变量list集合
    threshold:同值化处理的阈值
    ...
    const_col = []
    for col in col_list:
        df3 = df2[col].value_counts()
        const_pct = df3.iloc[0]/df2[df2[col].notnull()].shape[0]

        if const_pct>=threshold: #计算多数值占比超过90%的字段中，少数值的坏样本率是否会显著高于多数值,如果显著高则需要保留
            df4=df2[df2[col] == df3.index[0]]
            bad_rate_max = df4[target].sum()/ len(df4)

            df5=df2[df2[col] != df3.index[0]]
            bad_rate_min = df5[target].sum()/ len(df5)

            if bad_rate_min<bad_rate_max:
                const_col.append(col)
                #print(bad_rate_min,bad_rate_max)
    df2 = df2.drop(const_col,axis=1)
    print('常变量/同值化处理的变量个数为{};'.format(len(const_col)),const_col)
    return df2
